<template>
	<section class="rounded-2xl border border-[#c2d4cb] bg-white p-5">
		<h3 class="text-sm font-semibold uppercase tracking-widest text-[#0d2b0f]">SPECIFIC Filters</h3>

		<div class="mt-4 space-y-4">
			<div>
				<label class="text-xs font-semibold uppercase tracking-widest text-[#4a7060]">Focus</label>
				<div class="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
					<button
						v-for="card in specificCards"
						:key="card.value"
						class="rounded-xl border px-3 py-3 text-left transition"
						:class="
							selectedSpecific === card.value
								? 'border-[#0d2b0f] bg-[#0d2b0f] text-white'
								: 'border-[#c2d4cb] bg-[#f4f8f5] text-[#3d6455] hover:border-[#0d2b0f]'
						"
						@click="$emit('update:selectedSpecific', card.value)"
					>
						<p class="text-xs font-semibold uppercase tracking-wide">{{ card.title }}</p>
						<p class="mt-1 text-xs opacity-90">{{ card.description }}</p>
					</button>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
				<div>
					<label class="text-xs font-semibold uppercase tracking-widest text-[#4a7060]">College (Optional)</label>
					<select
...
const emit = defineEmits<{
	...
	'update:selectedSpecific': [value: SpecificType]
...
const specificCards: { value: SpecificType; title: string; description: string }[] = [
	{ value: 'top_students', title: 'Top Visiting Students', description: 'Ranked students by total visits and duration.' },
	{ value: 'peak_hours', title: 'Peak and Low Hours', description: 'Hourly traffic trend and peak/low summary.' },
	{ value: 'duration', title: 'Duration Analysis', description: 'Session duration trends and distribution.' },
	{ value: 'events', title: 'Events Report', description: 'Attendance totals grouped by event.' },
]
